...
MiMo-Code loads OpenCode-compatible plugins, but MiMo V2.5-Pro behaves best when the Sisyphus surface stays compact. When oh-my-openagent detects MiMo-Code (`MIMOCODE=1`), it automatically selects the MiMo-specific Sisyphus prompt, caps the agent output budget, and keeps `team_*` / background task tools out of the active Sisyphus tool surface.

Minimal MiMo-Code config:
...
For local checkout development, use a built file URL instead:
...
Minimal oh-my-openagent config for MiMo-Code:

```jsonc
{
  "agents": {
    "sisyphus": { "model": "mimo/mimo-auto" }
  }
}
```

With the plugin loaded, both forms work:

```bash
mimo run --agent sisyphus "reply exactly: ok"
mimo run --agent "Sisyphus - ultraworker" "reply exactly: ok"
```

If no `default_agent` is already set, oh-my-openagent sets MiMo-Code's default agent to the compact Sisyphus profile. Do not copy the full non-MiMo Sisyphus prompt into MiMo config; the MiMo profile is applied by runtime detection.

Xiaomi's OpenAI-compatible docs list MiMo V2.5-Pro at 1,048,576 context tokens and 131,072 max output tokens, but large context is not a reason to inflate the system prompt. The useful pattern is a short durable operating contract plus task-local evidence. Xiaomi also documents that agent products using tool calls in multi-turn histories should preserve `reasoning_content`; missing reasoning context can produce 400 responses or weaker instruction following. Prefer the MiMo OpenAI-compatible route for MiMo-Code and keep the message history intact.
