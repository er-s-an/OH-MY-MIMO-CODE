...
describe("resolveRunAgent", () => {
  ...
  it("uses MiMo env agent over OpenCode env agent", () => {
    // given
    const config = createConfig({ default_run_agent: "prometheus" })
    const env = { MIMOCODE_DEFAULT_AGENT: "Hephaestus", OPENCODE_DEFAULT_AGENT: "Atlas" }
...
describe("run environment setup", () => {
  ...
  let originalMimocodeHome: string | undefined
  let originalMimocodeClient: string | undefined
  let originalMimocodeRunMode: string | undefined
  let consoleErrorSpy: ReturnType<typeof spyOn>

  beforeEach(() => {
    originalClient = process.env.OPENCODE_CLIENT
    originalRunMode = process.env.OPENCODE_CLI_RUN_MODE
    originalMimocodeHome = process.env.MIMOCODE_HOME
    originalMimocodeClient = process.env.MIMOCODE_CLIENT
    originalMimocodeRunMode = process.env.MIMOCODE_CLI_RUN_MODE
    delete process.env.MIMOCODE_HOME
    delete process.env.MIMOCODE_CLIENT
    delete process.env.MIMOCODE_CLI_RUN_MODE
  ...
  afterEach(() => {
    ...
    if (originalMimocodeClient === undefined) {
      delete process.env.MIMOCODE_CLIENT
    } else {
      process.env.MIMOCODE_CLIENT = originalMimocodeClient
    }
    if (originalMimocodeRunMode === undefined) {
      delete process.env.MIMOCODE_CLI_RUN_MODE
    } else {
      process.env.MIMOCODE_CLI_RUN_MODE = originalMimocodeRunMode
    }
  ...
  it("sets MiMo run markers when running under MiMo-Code", async () => {
    //#given
    ...
    //#then
    expect(String(process.env.MIMOCODE_CLIENT)).toBe("run")
    expect(String(process.env.MIMOCODE_CLI_RUN_MODE)).toBe("true")
  })
})
