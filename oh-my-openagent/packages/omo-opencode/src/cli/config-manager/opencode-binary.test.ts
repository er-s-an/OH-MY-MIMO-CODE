...
describe("getOpenCodeVersion (installer)", () => {
  ...
  describe("#given MiMo runtime #when getOpenCodeVersion #then probes mimo first and initializes mimocode context", () => {
    it("maps the mimo command to the mimocode config context", async () => {
      process.env.MIMOCODE_HOME = "/tmp/mimo"
      spawnSpy.mockReturnValue(createProc({ output: { stdout: "0.1.0\n" } }))

      const result = await getOpenCodeVersion()

      expect(result).toBe("0.1.0")
      expect(spawnSpy).toHaveBeenCalledWith(["mimo", "--version"], {
        stdout: "pipe",
        stderr: "pipe",
      })
      expect(initConfigContextSpy).toHaveBeenCalledWith("mimocode", "0.1.0")
    })
  })
