...
describe("loadPluginConfig", () => {
  ...
  it("should load user config from MIMOCODE_HOME/config when running under MiMo-Code", async () => {
    // given
    const rootDir = mkdtempSync(join(tmpdir(), "omo-plugin-config-mimocode-"))
    const mimocodeHome = join(rootDir, "mimo-home")
    const mimocodeConfigDir = join(mimocodeHome, "config")
    const projectDir = join(rootDir, "project")

    tempDirs.push(rootDir)
    mkdirSync(mimocodeConfigDir, { recursive: true })
    mkdirSync(join(projectDir, ".opencode"), { recursive: true })

    writeJsonFile(join(mimocodeConfigDir, "oh-my-openagent.jsonc"), {
      agents: {
        sisyphus: {
    ...
    process.env.MIMOCODE_HOME = mimocodeHome
    ...
    // then
    expect(config.agents?.sisyphus?.model).toBe("mimo/mimo-auto")
    expect(config.agents?.sisyphus?.maxTokens).toBe(4096)
    expect(config.experimental?.max_tools).toBe(5)
  })
