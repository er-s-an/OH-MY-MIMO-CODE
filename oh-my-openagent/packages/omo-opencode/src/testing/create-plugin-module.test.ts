...
describe("createPluginModule()", () => {
  let originalMimocodeEnv: string | undefined
  let originalMimocodeHomeEnv: string | undefined
  let originalMimocodeConfigDirEnv: string | undefined

  beforeEach(() => {
    originalMimocodeEnv = process.env.MIMOCODE
    originalMimocodeHomeEnv = process.env.MIMOCODE_HOME
    originalMimocodeConfigDirEnv = process.env.MIMOCODE_CONFIG_DIR
  ...
  afterEach(() => {
    if (originalMimocodeEnv === undefined) {
      delete process.env.MIMOCODE
    } else {
      process.env.MIMOCODE = originalMimocodeEnv
    }

    if (originalMimocodeHomeEnv === undefined) {
      delete process.env.MIMOCODE_HOME
    } else {
      process.env.MIMOCODE_HOME = originalMimocodeHomeEnv
    }

    if (originalMimocodeConfigDirEnv === undefined) {
      delete process.env.MIMOCODE_CONFIG_DIR
    } else {
      process.env.MIMOCODE_CONFIG_DIR = originalMimocodeConfigDirEnv
    }
  })

  describe("#given no MiMo-Code environment", () => {
    it("#then startup initializes the OpenCode config context", async () => {
      // given
      ...
      // then
      expect(mockInitConfigContext).toHaveBeenCalledWith("opencode", null)
    })
  })

  describe("#given MiMo-Code environment variables are present", () => {
    it("#then startup initializes the MiMo-Code config context", async () => {
      // given
      const pluginModule = createTestPluginModule()
      ...
      // then
      expect(mockInitConfigContext).toHaveBeenCalledWith("mimocode", null)
    })
  })
