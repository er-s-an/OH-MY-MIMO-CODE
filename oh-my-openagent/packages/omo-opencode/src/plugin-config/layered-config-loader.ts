...
function getUserConfigLayers(): Array<{ readonly configDir: string; readonly configPath: string | null }> {
  const runtimeBinary = detectRuntimeBinary();
  const userConfigDirs = [...getOpenCodeConfigDirs({ binary: runtimeBinary })].reverse();
  log("Resolved user plugin config dirs", {
    runtimeBinary,
    configDirs: userConfigDirs,
  });
