import { getRuntimeServerPassword, getRuntimeServerUsername } from "./harness-runtime"
...
export function getServerBasicAuthHeader(): string | undefined {
  const password = getRuntimeServerPassword()
  if (!password) {
    return undefined
  }

  const username = getRuntimeServerUsername()
