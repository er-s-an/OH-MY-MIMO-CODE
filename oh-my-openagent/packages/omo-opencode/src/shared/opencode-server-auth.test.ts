...
describe("opencode-server-auth", () => {
  ...
  test("#given MiMo server password without username #when building auth header #then uses MiMo default username", () => {
    process.env.MIMOCODE_HOME = "/tmp/mimo"
    process.env.MIMOCODE_SERVER_PASSWORD = "secret"
    ...
    expect(result).toBe("Basic bWltb2NvZGU6c2VjcmV0")
  })
