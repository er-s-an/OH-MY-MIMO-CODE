...
describe("Agent Config Integration", () => {
  ...
  describe("Display name resolution", () => {
    test("returns correct display names for all builtin agents", () => {
      ...
      expect(displayNames).toContain(AGENT_DISPLAY_NAMES.sisyphus)
      expect(displayNames).toContain(AGENT_DISPLAY_NAMES.hephaestus)
      expect(displayNames).toContain(AGENT_DISPLAY_NAMES.prometheus)
      expect(displayNames).toContain(AGENT_DISPLAY_NAMES.atlas)
      expect(displayNames).toContain(AGENT_DISPLAY_NAMES.metis)
      expect(displayNames).toContain(AGENT_DISPLAY_NAMES.momus)
    ...
    test("handles lowercase keys case-insensitively", () => {
      ...
      expect(displayNames[0]).toBe(AGENT_DISPLAY_NAMES.sisyphus)
      expect(displayNames[1]).toBe(AGENT_DISPLAY_NAMES.atlas)
      expect(displayNames[2]).toBe(AGENT_DISPLAY_NAMES.sisyphus)
      expect(displayNames[3]).toBe(AGENT_DISPLAY_NAMES.atlas)
      expect(displayNames[4]).toBe(AGENT_DISPLAY_NAMES.prometheus)
      expect(displayNames[5]).toBe(AGENT_DISPLAY_NAMES.prometheus)
    })
  ...
  describe("End-to-end config flow", () => {
    test("old config migrates and displays correctly", () => {
      ...
      // then - display names are correct
      expect(sisyphusDisplay).toBe(AGENT_DISPLAY_NAMES.sisyphus)
      expect(prometheusDisplay).toBe(AGENT_DISPLAY_NAMES.prometheus)
    ...
    test("new config works without migration", () => {
      ...
      // then - display names are correct
      expect(sisyphusDisplay).toBe(AGENT_DISPLAY_NAMES.sisyphus)
      expect(atlasDisplay).toBe(AGENT_DISPLAY_NAMES.atlas)
    })
  })
})
