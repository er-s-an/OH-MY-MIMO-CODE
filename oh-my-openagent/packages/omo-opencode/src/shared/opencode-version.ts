...
import { detectRuntimeBinary, getRuntimeCliBinary, isMimocodeRuntime } from "./harness-runtime"
...
const defaultDeps: OpenCodeVersionDeps = {
  ...
  getBinaryPath: () => {
    const envPath = isMimocodeRuntime() ? process.env.MIMOCODE_BIN_PATH : process.env.OPENCODE_BIN_PATH
    if (envPath) return envPath
    return (globalThis as RuntimeWithBun).Bun?.which(getRuntimeCliBinary()) ?? null
  },
  exists: existsSync,
...
function parsePackageVersion(content: string): string | null {
  try {
    ...
    if (typeof name !== "string" || !/(opencode|mimo|mimocode)/i.test(name)) return null
...
export function getOpenCodeVersion(deps: Partial<OpenCodeVersionDeps> = {}): string | null {
  ...
  try {
    const result = resolvedDeps.execCommand(`${getRuntimeCliBinary(detectRuntimeBinary())} --version`, {
      encoding: "utf-8",
      timeout: 5000,
      stdio: ["pipe", "pipe", "pipe"],
    }).trim()
...
export function isOpenCodeVersionAtLeast(version: string): boolean {
  if (isMimocodeRuntime()) return true
