...
describe("opencode-version", () => {
  ...
  describe("getOpenCodeVersion", () => {
    ...
    test("reads adjacent MiMo package version before executing binary", () => {
      // given a mimocode package next to the resolved binary
      const calls: string[] = []
      const packageRoot = join("/tmp", "mimocode")
      const binaryPath = join(packageRoot, "bin", "mimo")
      const packagePath = join(packageRoot, "package.json")
      ...
      const result = getOpenCodeVersion({
        ...
        readText: (filePath) => {
          calls.push(`read:${filePath}`)
          return JSON.stringify({ name: "mimocode", version: "0.1.0" })
        },
        execCommand: () => {
          calls.push("exec")
          return "0.1.0"
    ...
    test("falls back to mimo binary in MiMo runtime", () => {
      // given MiMo runtime is active and no adjacent package version can be read
      const originalMimocodeHome = process.env.MIMOCODE_HOME
      ...
      try {
        ...
        // then the MiMo CLI fallback is used
        expect(result).toBe("0.1.0")
        expect(calls).toEqual(["mimo --version"])
      } finally {
        if (originalMimocodeHome === undefined) {
          delete process.env.MIMOCODE_HOME
        } else {
          process.env.MIMOCODE_HOME = originalMimocodeHome
        }
      }
    })
  ...
  describe("isOpenCodeVersionAtLeast", () => {
    ...
    test("returns true for MiMo runtime even when MiMo semver is below OpenCode minimum", () => {
      // given MiMo runtime is active
      const originalMimocodeHome = process.env.MIMOCODE_HOME
      process.env.MIMOCODE_HOME = "/tmp/mimo"
      setVersionCache("0.1.0")

      try {
        // when checking against an OpenCode version gate
        const result = isOpenCodeVersionAtLeast("1.4.0")

        // then MiMo is not rejected by OpenCode-only version gates
        expect(result).toBe(true)
      } finally {
        if (originalMimocodeHome === undefined) {
          delete process.env.MIMOCODE_HOME
