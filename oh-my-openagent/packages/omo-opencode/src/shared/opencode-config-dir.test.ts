...
describe("opencode-config-dir", () => {
  ...
  describe("getOpenCodeConfigPaths", () => {
    ...
    test("returns mimocode config paths for MiMo-Code binary", () => {
      // given MiMo-Code CLI binary on Linux
      Object.defineProperty(process, "platform", { value: "linux" })
      delete process.env.XDG_CONFIG_HOME
      ...
      // when getOpenCodeConfigPaths is called
      const paths = getOpenCodeConfigPaths({ binary: "mimocode", version: "0.1.0" })

      // then returns MiMo-Code config filenames in the mimocode config namespace
      const expectedDir = join(homedir(), ".config", "mimocode")
      expect(paths.configDir).toBe(expectedDir)
      expect(paths.configJson).toBe(join(expectedDir, "mimocode.json"))
      expect(paths.configJsonc).toBe(join(expectedDir, "mimocode.jsonc"))
      expect(paths.packageJson).toBe(join(expectedDir, "package.json"))
      expect(paths.omoConfig).toBe(join(expectedDir, "oh-my-openagent.json"))
    })

    test("prefers MIMOCODE_HOME/config for MiMo-Code binary", () => {
      // given MiMo-Code home is set
      Object.defineProperty(process, "platform", { value: "linux" })
      process.env.MIMOCODE_HOME = "/tmp/mimocode-home"
      process.env.MIMOCODE_CONFIG_DIR = "/custom/mimocode/path"

      // when getOpenCodeConfigPaths is called
      const paths = getOpenCodeConfigPaths({ binary: "mimocode", version: "0.1.0" })

      // then returns MiMo-Code's real config root
      const expectedDir = join(resolve("/tmp/mimocode-home"), "config")
      expect(paths.configDir).toBe(expectedDir)
      expect(paths.configJson).toBe(join(expectedDir, "mimocode.json"))
      expect(paths.configJsonc).toBe(join(expectedDir, "mimocode.jsonc"))
    })
