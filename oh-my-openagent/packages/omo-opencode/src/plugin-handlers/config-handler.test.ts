...
describe("default_agent behavior with Sisyphus orchestration", () => {
  ...
  test("sets compact sisyphus as default_agent under MiMo-Code when missing", async () => {
    // #given
...
describe("per-agent todowrite/todoread deny when task_system enabled", () => {
  ...
  test("keeps nested Sisyphus task and team tools denied under MiMo-Code", async () => {
    //#given
    const originalMimocode = process.env.MIMOCODE
    process.env.MIMOCODE = "1"
    const createBuiltinAgentsMock = unsafeTestValue<{
    ...
    try {
      ...
      //#then
      const agentResult = config.agent as Record<string, { permission?: Record<string, unknown> }>
      const sisyphus = agentResult[getAgentListDisplayName("sisyphus")] ?? agentResult.sisyphus
      expect(sisyphus?.permission).toMatchObject({
        "*": "deny",
        read: "allow",
        task: "deny",
        "task_*": "deny",
