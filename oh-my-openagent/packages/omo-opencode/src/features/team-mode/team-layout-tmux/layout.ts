import { getRuntimeCliBinary, getRuntimeServerEnvVars, log } from "../../../shared"
...
function buildAttachCommand(member: TeamLayoutMember, serverUrl: string): string {
  return `${getRuntimeCliBinary()} attach ${shellSingleQuote(serverUrl)} --session ${shellSingleQuote(member.sessionId)} --dir ${shellSingleQuote(getPaneWorkingDirectory(member))}`
}

function buildPaneEnvironmentArgs(): string[] {
  return Object.entries(getRuntimeServerEnvVars()).flatMap(([key, value]) => ["-e", `${key}=${value}`])
}
