...
describe("team-layout-tmux", () => {
  ...
  test("#given MiMo runtime auth set #when createTeamLayout runs #then panes use mimo attach and MiMo auth env", async () => {
    // given
    const originalHome = process.env.MIMOCODE_HOME
    const originalPwd = process.env.MIMOCODE_SERVER_PASSWORD
    const originalUser = process.env.MIMOCODE_SERVER_USERNAME
    process.env.MIMOCODE_HOME = "/tmp/mimo"
    process.env.MIMOCODE_SERVER_PASSWORD = "pw"
    process.env.MIMOCODE_SERVER_USERNAME = "mi"
    try {
      const { createTeamLayout } = await loadLayoutModule()
      const members = [{ name: "m1", sessionId: "s-m1", worktreePath: "/tmp/m1" }]

      // when
      await createTeamLayout("run-mimo-auth", members, tmuxMgr as never)

      // then
      const commands = getCommands()
      const sendKeys = commands.filter((args) => args[0] === "send-keys").map((args) => args.join(" "))
      expect(sendKeys.some((s) => s.includes("mimo attach"))).toBe(true)
      const splitWindow = commands.find((args) => args[0] === "split-window")
      expect(splitWindow).toContain("MIMOCODE_SERVER_PASSWORD=pw")
      expect(splitWindow).toContain("MIMOCODE_SERVER_USERNAME=mi")
    } finally {
      if (originalHome === undefined) delete process.env.MIMOCODE_HOME; else process.env.MIMOCODE_HOME = originalHome
      if (originalPwd === undefined) delete process.env.MIMOCODE_SERVER_PASSWORD; else process.env.MIMOCODE_SERVER_PASSWORD = originalPwd
      if (originalUser === undefined) delete process.env.MIMOCODE_SERVER_USERNAME; else process.env.MIMOCODE_SERVER_USERNAME = originalUser
    }
  })
