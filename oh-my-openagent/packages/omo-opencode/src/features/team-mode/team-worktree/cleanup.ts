...
export async function removeWorktree(worktreePath: string): Promise<void> {
  ...
  const gitCommonDir = rootStdout.trim()
  const repositoryRoot =
    rootExitCode === 0 && gitCommonDir.length > 0
  ...
  const result =
    await runGit(["-C", repositoryRoot, "worktree", "remove", "--force", worktreePath])
  ...
  await runGit(["-C", repositoryRoot, "worktree", "prune"])
}
