...
export function resolveServerUrl(
  ...
  const configuredPort = env.MIMOCODE_PORT ?? env.OPENCODE_PORT
      ...
      if (port === '0') {
        log(
          "[tmux-session-manager] ctx.serverUrl has port 0; falling back. " +
            ...
            "Launch the CLI with --port N and OPENCODE_PORT=N or MIMOCODE_PORT=N to bind a real port (see issue #3963).",
